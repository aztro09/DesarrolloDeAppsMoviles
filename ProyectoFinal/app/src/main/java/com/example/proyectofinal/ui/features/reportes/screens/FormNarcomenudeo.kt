...
@Composable
fun FormNarcomenudeo(
    vm: ReportesViewModel,
    onEnviado: (String) -> Unit
){
    var actividad by remember {mutableStateOf("venta")}
    ...
    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        TextField(
            ...
            label = { Text("Actividad sospechosa") })
        TextField(
            value = descripcion,
            onValueChange = { descripcion = it },
            label = { Text("Descripción de personas/vehículos") })
        Row {
            TextField(
                value = lat.toString(),
        ...
        Button(onClick = {
            val r = Reporte(
                categoria = Categoria.NARCOMENUEDEO,
                subtipo = actividad,
