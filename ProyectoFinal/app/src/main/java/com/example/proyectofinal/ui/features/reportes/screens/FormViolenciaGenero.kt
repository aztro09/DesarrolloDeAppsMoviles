...
@Composable
fun FormViolenciaGenero(
    vm: ReportesViewModel,
    onEnviado: (String) -> Unit
) {
    var tipo by remember { mutableStateOf("fisica") }
    var relacion by remember { mutableStateOf("pareja") }
    ...
    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(onClick = {
            val r = Reporte(
                categoria = Categoria.VIOLENCIA_GENERO,
                subtipo = tipo,
                descripcion = "$descripcion (relacion: $relacion)",
                lat = lat, lng = lng
            )
