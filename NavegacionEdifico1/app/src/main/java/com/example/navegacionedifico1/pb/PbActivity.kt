package com.example.navegacionedifico1.pb
...
class PbActivity : FragmentActivity() {
    override fun onCreate(savedInstanceState: Bundle?){
        super.onCreate(savedInstanceState)
        setContent{
            PbScreen(onSalonClick = { salonNumber ->
...
@Composable
fun PbScreen(onSalonClick: (Int) -> Unit) {
    Scaffold(
        topBar = {
            TopAppBar(title = { Text("Planta Baja (PB)") })
        }
    ){ padding ->
        LazyColumn (
            modifier = Modifier
